...
from trove.extensions.k2hdkc.common import populate_users
from trove.extensions.k2hdkc.common import populate_validated_databases
from trove.extensions.k2hdkc.common import unquote_user_host
from trove.extensions.k2hdkc import models
from trove.extensions.k2hdkc import views
...
#
# This is a file that has not yet been examined!
#
# The implementation is modeled after MySQL.
# Therefore, the following functions are currently being called:
#
def populate_validated_databases(dbs):
    ...
    try:
        ...
        for database in dbs:
            mydb = guest_models.K2hdkcSchema(name=database.get('name', ''))
            mydb.check_reserved()
...
def populate_users(users, initial_databases=None):
    ...
    unique_identities = set()
    for user in users:
        u = guest_models.K2hdkcUser(name=user.get('name', ''),
                                   host=user.get('host', '%'))
        u.check_reserved()
...
class UserController(ExtensionController):
    ...
    def delete(self, req, tenant_id, instance_id, id):
        ...
        with StartNotification(context, instance_id=instance_id,
                               username=username):
            try:
                user = guest_models.K2hdkcUser(name=username,
                                              host=host)
    ...
    def update_all(self, req, body, tenant_id, instance_id):
        ...
        with StartNotification(context, instance_id=instance_id,
            ...
            for user in users:
                try:
                    mu = guest_models.K2hdkcUser(name=user['name'],
                                                host=user.get('host'),
...
class SchemaController(ExtensionController):
    ...
    def delete(self, req, tenant_id, instance_id, id):
        ...
        with StartNotification(context, instance_id=instance_id, dbname=id):
            try:
                schema = guest_models.K2hdkcSchema(name=id)
                schema.check_delete()
